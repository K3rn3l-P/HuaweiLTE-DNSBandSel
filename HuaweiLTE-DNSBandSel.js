    ...
    $.ajax({
      ...
      success: function (n) {
        for (
          ...
          (window[vars[i]] = extractXML(vars[i], n)),
            $("#" + vars[i]).html(window[vars[i]]);
        (nrdefined = "undefined" != typeof nrrsrp),
          $(".e5").toggle(nrdefined),
          nrdefined &&
            (setgraph("nrrsrp", nrrsrp, -130, -70),
            setgraph("nrrsrq", nrrsrq, -16, -3)),
          setgraph("rsrp", rsrp, -130, -70),
          setgraph("rsrq", rsrq, -16, -3),
          (mp = cell_id.indexOf("-")),
          (enbid =
            0 < mp
              ? Number(cell_id.substr(0, mp))
              : ((hex = Number(cell_id).toString(16)),
                (hex2 = hex.substring(0, hex.length - 2)),
                parseInt(hex2, 16).toString())),
          $("#enbid").html(enbid),
...
function getAntenna() {
  $.ajax({
    ...
    error: err,
    success: function (n) {
      (antenna1type = extractXML("antenna1type", n)),
        (antenna2type = extractXML("antenna2type", n)),
        "1" == antenna1type ? $("#a1").html("EXT") : $("#a1").html("INT"),
        "1" == antenna2type ? $("#a2").html("EXT") : $("#a2").html("INT");
    },
  });
}
function getNetmode() {
...
function getStatus() {
  $.ajax({
    ...
    success: function (n) {
      (status = n),
        (is4gp = 1011 == extractXML("CurrentNetworkTypeEx", n) ? 1 : 0),
        is4gp
          ? $("#mode").html("4G+").css("color", "red")
          : $("#mode").html("-").css("color", "#aaa");
    },
  });
}
function err(n, e, r) {
...
function setgraph(n, r, t, a) {
  (r = parseInt(r.replace("dBm", "").replace("dB"))),
    (x = ((r - t) / (a - t)) * 100),
    (xs = String(x) + String.fromCharCode(37)),
    (e = "#" + n + "b"),
    $(e).animate({ width: xs }),
    $(e).html(n.replace("nr", "5G - ") + " : " + window[n]),
    x < 50
...
function _4GType(n) {
  for (data_out = "", x = 0; x < 90; x++)
    (tb = Math.pow(2, x)),
      (color =
        BigInt("0x" + n) & BigInt(tb)
          ? ((data_out += "B" + String(x + 1) + "+"), "#686")
          : "transparent"),
      $("#cb" + String(x + 1)).css("background-color", color);
...
function ltebandselection(n) {
  if (((mainband = mainband && null), 0 == arguments.length)) {
    if (
      null ==
        (e =
          (e = prompt(
            "Please input LTE bands number, separated by + char (example 1+3+20).If you want to use every supported bands, write 'AUTO'.",
            "AUTO"
          )) && e.toLowerCase()) ||
      "" === e
    )
  ...
  if ("AUTO" === e.toUpperCase()) t = "7FFFFFFFFFFFFFFF";
    ...
    for (var r = 0; r < n.length; r++) {
      if (
        (-1 != n[r].toLowerCase().indexOf("m") &&
          ((n[r] = n[r].replace("m", "")), (mainband = n[r])),
        "AUTO" === n[r].toUpperCase())
      ) {
        t = "7FFFFFFFFFFFFFFF";
        break;
      }
    ...
    $.ajax({
      ...
      success: function (n) {
        var n = n.split('name="csrf_token" content="'),
          ...
          r = "00";
        $("#force4g").is(":checked") && (r = "03"),
          setTimeout(function () {
            $.ajax({
              type: "POST",
              async: !0,
              url: "/api/net/net-mode",
              headers: { __RequestVerificationToken: e },
              contentType: "application/xml",
              data:
                "<request><NetworkMode>" +
                r +
                "</NetworkMode><NetworkBand>3FFFFFFF</NetworkBand><LTEBand>" +
                t +
                "</LTEBand></request>",
              success: function (n) {
...
(mainband = null),
  ...
  (signal = ""),
  (version = "4.0b"),
  console.log("Code by K3rn3l - p" + version),
